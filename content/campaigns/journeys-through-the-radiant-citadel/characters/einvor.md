---
title: Einvor
date: 2026-07-09 19:00:00+00:00
params:
  kind: character
draft: false
weight: 10
summary: Goliath Barbarian 2 importado do D&D Beyond.
tags:
- jogador
- goliath
- barbarian
visibility: players
status: ready
char_info:
  class: Barbarian 2
  species: Goliath
  ac: '13'
  hp: '23'
  hp_max: '23'
  hp_current: '23'
  feat: Weapon Mastery
  size: Medium
  alignment: Neutral
  dndbeyond_id: '168143718'
  proficiency_bonus: 2
  spell_dc: 0
  spell_attack_bonus: 0
  avatar: /images/campaigns/journeys-through-the-radiant-citadel/handouts/einvor.png
  speed:
    walk: 35
    fly: 0
    swim: 0
    climb: 0
    burrow: 0
  senses: Passive Perception 10
  passive_senses:
    perception: 10
    investigation: 9
    insight: 10
  languages: Common, Giant, Orc, Sylvan
  saves:
    str: 4
    dex: 1
    con: 4
    int: -1
    wis: 0
    cha: 0
  saves_proficient:
    str: true
    dex: false
    con: true
    int: false
    wis: false
    cha: false
  saves_summary: Str +4, Con +4
  mods:
    str: 2
    dex: 1
    con: 2
    int: -1
    wis: 0
    cha: 0
  stats:
    str: 14
    dex: 12
    con: 14
    int: 8
    wis: 10
    cha: 10
  currencies:
    cp: 0
    sp: 0
    gp: 25
    ep: 0
    pp: 0
  skills:
    acrobatics:
      bonus: 1
      proficient: false
      expertise: false
      stat: dex
    animal-handling:
      bonus: 0
      proficient: false
      expertise: false
      stat: wis
    arcana:
      bonus: -1
      proficient: false
      expertise: false
      stat: int
    athletics:
      bonus: 4
      proficient: true
      expertise: false
      stat: str
    deception:
      bonus: 0
      proficient: false
      expertise: false
      stat: cha
    history:
      bonus: -1
      proficient: false
      expertise: false
      stat: int
    insight:
      bonus: 0
      proficient: false
      expertise: false
      stat: wis
    intimidation:
      bonus: 2
      proficient: true
      expertise: false
      stat: cha
    investigation:
      bonus: -1
      proficient: false
      expertise: false
      stat: int
    medicine:
      bonus: 0
      proficient: false
      expertise: false
      stat: wis
    nature:
      bonus: 1
      proficient: true
      expertise: false
      stat: int
    perception:
      bonus: 0
      proficient: false
      expertise: false
      stat: wis
    performance:
      bonus: 0
      proficient: false
      expertise: false
      stat: cha
    persuasion:
      bonus: 0
      proficient: false
      expertise: false
      stat: cha
    religion:
      bonus: -1
      proficient: false
      expertise: false
      stat: int
    sleight-of-hand:
      bonus: 1
      proficient: false
      expertise: false
      stat: dex
    stealth:
      bonus: 1
      proficient: false
      expertise: false
      stat: dex
    survival:
      bonus: 2
      proficient: true
      expertise: false
      stat: wis
  actions:
  - name: Ataque
    ref: /compendium/rules/action-attack/
    max_uses: 0
    reset: ''
  - name: Esconder
    ref: /compendium/rules/action-hide/
    max_uses: 0
    reset: ''
  - name: Desengajar
    ref: /compendium/rules/action-disengage/
    max_uses: 0
    reset: ''
  - name: Disparar
    ref: /compendium/rules/action-dash/
    max_uses: 0
    reset: ''
  - name: Ajudar
    ref: /compendium/rules/action-help/
    max_uses: 0
    reset: ''
  - name: Esquivar
    ref: /compendium/rules/action-dodge/
    max_uses: 0
    reset: ''
  - name: Usar Objeto
    ref: /compendium/rules/action-use-object/
    max_uses: 0
    reset: ''
  - name: Rage (Enter)
    max_uses: 2
    reset: Descanso Longo
    source: class
    ref: /compendium/rules/rage-enter/
  - name: Hill’s Tumble (Hill Giant)
    max_uses: 0
    reset: Descanso Longo
    source: race
    ref: /compendium/rules/hills-tumble-hill-giant/
  - name: Cleave (Greataxe)
    max_uses: 0
    reset: ''
    source: feat
    ref: /compendium/rules/cleave-greataxe/
  - name: Sap (Mace)
    max_uses: 0
    reset: ''
    source: feat
    ref: /compendium/rules/sap-mace/
  equipment:
  - name: Handaxe
    quantity: 1
    equipped: false
    filter_type: Weapon
    attack_formula: 1d20 + 2
    damage_formula: 1d6 + 2
    ref: /compendium/items/handaxe/
  - name: Handaxe
    quantity: 1
    equipped: false
    filter_type: Weapon
    attack_formula: 1d20 + 2
    damage_formula: 1d6 + 2
    ref: /compendium/items/handaxe/
  - name: Handaxe
    quantity: 1
    equipped: false
    filter_type: Weapon
    attack_formula: 1d20 + 2
    damage_formula: 1d6 + 2
    ref: /compendium/items/handaxe/
  - name: Handaxe
    quantity: 1
    equipped: false
    filter_type: Weapon
    attack_formula: 1d20 + 2
    damage_formula: 1d6 + 2
    ref: /compendium/items/handaxe/
  - name: Quarterstaff
    quantity: 1
    equipped: false
    filter_type: Weapon
    attack_formula: 1d20 + 2
    damage_formula: 1d6 + 2
    ref: /compendium/items/quarterstaff/
  - name: Greataxe
    quantity: 1
    equipped: true
    filter_type: Weapon
    attack_formula: 1d20 + 4
    damage_formula: 1d12 + 2
    ref: /compendium/items/greataxe/
  - name: Clothes, Traveler's
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
  - name: Hunting Trap
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/hunting-trap/
  - name: Pouch
    quantity: 1
    equipped: true
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/pouch/
  - name: Backpack
    quantity: 1
    equipped: true
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/backpack/
  - name: Oil
    quantity: 2
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/oil/
  - name: Rations
    quantity: 10
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/rations/
  - name: Rope
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/rope/
  - name: Bedroll
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/bedroll/
  - name: Tinderbox
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/tinderbox/
  - name: Torch
    quantity: 10
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/torch/
  - name: Waterskin
    quantity: 1
    equipped: false
    filter_type: Other Gear
    attack_formula: ''
    damage_formula: ''
    ref: /compendium/items/waterskin/
  spells: []
  classes_progression:
  - name: Barbarian
    level: 2
    subclass: ''
locations: []
factions: []
compendium_refs:
- /compendium/classes/barbarian/
- /compendium/feats/weapon-master/
- /compendium/items/backpack/
- /compendium/items/bedroll/
- /compendium/items/greataxe/
- /compendium/items/handaxe/
- /compendium/items/hunting-trap/
- /compendium/items/oil/
- /compendium/items/pouch/
- /compendium/items/quarterstaff/
- /compendium/items/rations/
- /compendium/items/rope/
- /compendium/items/tinderbox/
- /compendium/items/torch/
- /compendium/items/waterskin/
- /compendium/species/goliath/
- /compendium/rules/action-attack/
- /compendium/rules/action-dash/
- /compendium/rules/action-disengage/
- /compendium/rules/action-dodge/
- /compendium/rules/action-help/
- /compendium/rules/action-hide/
- /compendium/rules/action-use-object/
- /compendium/rules/cleave-greataxe/
- /compendium/rules/danger-sense/
- /compendium/rules/hills-tumble-hill-giant/
- /compendium/rules/rage-enter/
- /compendium/rules/rage/
- /compendium/rules/reckless-attack/
- /compendium/rules/sap-mace/
- /compendium/rules/unarmored-defense/
- /compendium/rules/weapon-mastery/
spells_usage: []
---
### Biografia
Este personagem foi importado automaticamente do D&D Beyond. 

### Equipamentos e Recursos
Acesse a ficha completa original no D&D Beyond para acompanhar o inventário de itens e slots de magia em tempo real.

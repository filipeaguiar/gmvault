...
### 5. Script de Sincronização Local com 5e.tools (`sync_character.py`)
- **Opção Escolhida**: Criação de um script Python `scripts/sync_character.py` que aceita o caminho de um arquivo Markdown de personagem como parâmetro. O script extrai a lista de magias conhecidas, equipamentos, talentos e classes do frontmatter, detecta novos itens inseridos manualmente que não constam em `compendium_refs`, busca os dados ausentes nos servidores do 5e.tools, gera os respectivos arquivos estáticos locais no compêndio e atualiza a chave `compendium_refs` no próprio Markdown do personagem.
- **Alternativa Considerada**: Executar uma varredura completa toda vez que o Hugo compilar o site. Embora automatizada, isso violaria o fluxo de compilação estática rápida do Hugo, pois Hugo não possui recursos nativos para fazer requisições HTTP e gravar arquivos locais durante o build de páginas. O script Python desacoplado em tempo de edição é a única forma viável e eficiente de manter o compêndio atualizado sem comprometer a performance do build estático.
...
- **[Risco] Corrupção do Formato do Markdown ao Re-gravar o Frontmatter**: Re-gravar o arquivo Markdown usando bibliotecas Python simples pode corromper comentários, formatação customizada ou a biografia escrita em prosa abaixo das linhas delimitadoras de YAML `---`.
  - *Mitigação*: O script `sync_character.py` deve carregar o frontmatter usando a biblioteca PyYAML, atualizar a lista `compendium_refs` e remontar o arquivo de forma cirúrgica: re-gerando o frontmatter em string YAML delimitado por `---`, seguido pela preservação intocada de todo o conteúdo e biografia que existia abaixo do segundo delimitador `---` no arquivo original.
